...
| Requirement | Status |
|---|---|
| Integration with an existing Stellar protocol (load-bearing) | ✅ **Blend v2.** The gate reads the pool's reserves through `get_reserve_list()` and builds its deposit as Blend's own `submit(from, spender, to, requests)` with `RequestType::Supply` ([web/gate/lib/protocol.js](web/gate/lib/protocol.js)); the pool is `CCEBVDYM…44HGF`. Wallet connection is Stellar Wallets Kit 2.6.0, pinned. Load-bearing in the strict sense: without the pool call there is nothing for the gate to gate. **What has not run: a successful `submit()`.** [`supply()`](web/gate/lib/protocol.js) re-checks the verdict and throws on anything but `Verified`, and the configured pool reads `NoClaim` — so the gate refuses it *by design* ([config.js](web/gate/config.js) says so). The refusal is what was demonstrated end to end; the success branch is exercised by neither chain nor test ([protocol.test.js](web/gate/test/protocol.test.js) loops the five non-`Verified` states only) |
...
| Core feature | ✅ **The check is the product, not a demo wrapped around one.** `supply()` re-reads the registry verdict and throws on anything but `Verified` *before* it will build the Blend `submit()` ([`protocol.js`](web/gate/lib/protocol.js)); exactly one of the registry's six states opens the gate ([`consensus.js`](web/gate/lib/consensus.js)), and an unrecognised value throws rather than degrading to "not verified". Read live from [sorofy.site/gate/](https://sorofy.site/gate/): the Blend v2 pool is refused (`NoClaim`), VRFY passes (`Verified`). `cd contracts && cargo test` → **46 passed**; `npm test` in [web/gate](web/gate/README.md) → **42 passed**. **The limit:** the staked set is three identities this team funded on one machine — follower mode does not exist, so this is a mechanism proven on chain, not an independent operator set |
...
|---|---|---|
| [`stellar/stellar-dev-skill`](https://github.com/stellar/stellar-dev-skill) | The official plugin — author *Stellar Development Foundation*, Apache 2.0, `stellar-dev` v1.2.0 at commit `202be80` (2026-09-14). Ships 8 skills: `smart-contracts`, `standards`, `dapp`, `assets`, `agentic-payments`, `cross-chain`, `data`, `zk-proofs` | **No — not installed.** The SEP claims here were re-checked directly instead: SEP-41 against the deployed VRFY token, SEP-1/SEP-10/SEP-24 against `testanchor.stellar.org` through [`anchor.js`](web/gate/lib/anchor.js). Nothing in this file rests on the plugin. The handbook asks which skill files were used during development: **none were** |
